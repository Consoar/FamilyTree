...
/**
 * Generic View for panning and zooming a canvas that is drawn on
 * 
 * Props to https://github.com/ruffythepirate/MindMap/blob/master/src/se/jc/library/graphics/ZoomView.java
 *  and http://www.ruffythepirate.com/?p=128&preview=true#Zooming_to_where_you_pinch
 *  where this code was heavily borrowed from
 */
public class ZoomView extends View {
    ...
    public float getDragDistanceFromStart(MotionEvent event) {
        ...
        if (dragStart != null) {
            return (float) getDistance(dragStart, new PointF(event.getX(), event.getY()));
        }
        return 0.0f;
    }
    
    private double getDistance(PointF pointOne, PointF pointTwo)
    {
    ...
    // on a Galaxy S3 (but worked fine on an older device) - the metrics were off, and strange things were happening to the 
    // clip bounds.  So don't use this, use canvas.save/restore while setting the scale/translate and drawing
    // in between.  The Android docs don't recommend messing with the Matrix directly, and have deprecated the getMatrix api
